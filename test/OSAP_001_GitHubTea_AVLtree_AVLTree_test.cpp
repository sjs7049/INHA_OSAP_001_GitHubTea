...
void AVLTreeTestFixture::SetUp()
{
    ...
    for (const int key_ : test_keys)
    {
        avl_tree_.Insert(key_); // avl tree에 key 삽입
    }
    std::cout<<"⚡ 테스트를 시작합니다. ⚡\n";
}
...
void AVLTreeTestFixture::TearDown()
{
    ...
    for (const int key_ : test_keys)
    {
        avl_tree_.Erase(key_); // avl tree로부터 모든 key 삭제
    }
...
TEST_F(AVLTreeTestFixture, AncestorKeySumMethodTest)
{
    ...
    EXPECT_EQ(avl_tree_.AncestorKeySum(234191), 100241); //1개의 ancestor를 가질 경우에 대한 test
    EXPECT_EQ(avl_tree_.AncestorKeySum(100241), 0); //2개 이상의 ancestor를 가질 경우에 대한 test
}
...
TEST_F(AVLTreeTestFixture, BalanceMethodTest)
{
    ...
    int initial_balance = avl_tree_.Difference(avl_tree_.root_);  // 불균형 트리일 경우 1보다 크거나 -1보다 작음
...
TEST_F(AVLTreeTestFixture, SearchMethodTest)
{
    ...
    EXPECT_EQ(found_node->key_, 50012);  // 찾은 노드가 올바른 key를 가지고 있어야 함

    Node* not_found_node = avl_tree_.Search(avl_tree_.root_, 999999);  // 존재하지 않는 key

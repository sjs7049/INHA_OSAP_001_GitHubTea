...
public:
  ...
  bool IsRoot(int key);              // AVL Tree에 매개변수 key값을 가진 노드가 루트 노드인지 판별하여 bool로 return
  bool IsExist(int key);             // AVL Tree에 매개변수 key값을 가진 노드가 존재하는지 판별하여 bool로 return
  ...
  int Depth(int key);                // AVL Tree의 매개변수 key값을 가진 노드를 찾아 깊이를 int로 return
  int Find(int key);                 // AVL Tree의 매개변수 key값을 가진 노드를 찾아 깊이와 높이의 합을 출력

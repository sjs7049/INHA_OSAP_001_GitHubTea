...
Node* AVLTree::ll(Node* parent) {
	...
	if (temp->right_node_() != NULL) temp->right_node_()->parent_node_set(parent);
...
bool AVLTree::isExist(int key) {
	return (search(root, key) != NULL ? 1 : 0);
}
...
void AVLTree::Insert(int key) {
	...
	while (curNode != NULL) {
		parNode = curNode;
		if (curNode->key_() < key) curNode = curNode->right_node_();
		else curNode = curNode->left_node_();
	}
...
void AVLTree::Erase(int key) {
	...
	if (delNode->left_node_() == nullptr && delNode->right_node_() == nullptr) {
		...
		childNode = delNode->right_node_();
		while (childNode->left_node_() != nullptr) childNode = childNode->left_node_();
	...
	if (parNode == nullptr) root = childNode;
